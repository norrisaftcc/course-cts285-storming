...
| **Anecdote + forward-reference review of `drafts/cts285/**`** | Never owned. B-003 audited the planning and canon layer only (its own record says so); B-005 removed two fabricated anecdotes from Week 2 only because that week was being rewritten anyway | `F-006` class 8: **8** `I've seen/watched students…` claims across 7 files, of which at least one carried invented particulars (fixed 2026-08-07). `F-006` class 7: `in Week N` forward references are unchecked against the spine, and at least one pointed at weeks that do not exist. **One grep, one pattern — a lower bound, not an inventory.** | **Unassigned.** Natural home is the `drafts/cts285/**` sweep B-003 never ran | 2026-08-07 |
