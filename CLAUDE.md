...
1. `phase0/` — ALL of it. These are hard constraints, not background:
   ...
   - `ADR-002-points-are-header-level.md` — points bind at the assignment header; sub-items are descriptive (folded into non-negotiable #1 below)
   ...
   - `ADR-004-two-modernization-bases.md` — Dataman *and* DataMon are both student bases; exemplars use Dataman (folded into non-negotiable #2 below)
   ...
   - `ADR-007-trusted-workflow-verbs.md` — the principle that the Trusted Workflow gets dedicated UPPERCASE verbs in a checkmarked form is **Accepted**; the seven-verb lexicon itself is **Proposed**. Until ratified, keep using the canonical step names — a half-converted lexicon is worse than either state
