...
| **Conflict (with teacher)** | "The clock is **deliberately imprecise**... keep the ticks weird." vs teacher's need for consistent, fair classroom timing. | Surface the tension (Story 7.2 flags it). Do not pre-decide; a configurable/"classic vs classroom" resolution is defensible. |
...
2. **Atom-Clock timing: imprecise "character" vs. consistent fairness.** Collector wants ticks kept deliberately variable; teacher wants consistent timing for classroom competition. **Guidance:** a configurable / dual-mode resolution ("classic ticks" vs "classroom-consistent") is strong; pre-deciding without noticing the conflict is weak.
...
3. **No core story is motivated only by the instructor-as-client persona.** *(Updated 2026-08-07: the persona is no longer deferred — `Client_Brief_Instructor_As_Client.md` was authored under ADR-003's remainder. The guarantee now holds **by construction rather than by absence**: the brief commissions the modernization and requests no features, so every core story still traces to Renee, Ms. Alvarez, or Dale.)* Per `ADR-003:48`, any future revision that gives the client a feature request of its own owes this backlog a story and this guarantee a rewrite — both in the same change.
4. **Every stretch epic traces to a named feature of the legacy creature-collection game**, as recorded in `phase0/NAMING_CANON.md` §"Dataman vs. DataMon" (legacy-game row) and ruled by `phase0/ADR-004-two-modernization-bases.md`: S1.1→save/load, S1.2→turn-based battle, S1.3→random encounters and progression, S1.4→creature collection. *(Re-pointed 2026-08-07. The prior citation was `CTS285_COURSE_OUTLINE.md` lines 168–185; that file was **retired at ingestion** (`planning/INGESTION_MANIFEST.md:332`) and exists nowhere in the tree — not in `sources/` either — so the guarantee was uncheckable. Features are named rather than line-numbered on purpose: a line number into a live canon file moves the first time anyone edits that table, which is how this defect was built.)*
5. **No orphan stories.** The seeded backlog contains no story that no transcript (or, for stretch, no named legacy-game feature in NAMING_CANON's legacy-game row) motivates.
