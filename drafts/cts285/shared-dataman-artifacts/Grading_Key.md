...
4. **Every stretch epic traces to a line in `CTS285_COURSE_OUTLINE.md` lines 168–185** (the legacy creature-game feature list): S1.1→save/load (l.171), S1.2→turn-based battle (l.172), S1.3→random encounters & progression (l.173), S1.4→creature collection (l.170).
5. **No orphan stories.** The seeded backlog contains no story that no transcript (or, for stretch, no outline line) motivates.
