...
2. **Every top-of-backlog (core) story has a named motivating source.** Each transcript-derived core story carries a `↳ Traces to:` line naming the stakeholder(s) and manual page; Epic 8 is Appendix-derived and does not claim stakeholder motivation.
3. **No core story is motivated only by the instructor-as-client persona.** *(Updated 2026-08-07: the persona is no longer deferred — `Client_Brief_Instructor_As_Client.md` was authored under ADR-003's remainder. The guarantee now holds **by construction rather than by absence**: the brief commissions the modernization and requests no features, so every core story still traces to Renee, Ms. Alvarez, or Dale, except the explicit manual-Appendix Epic 8 exception.)* Per `ADR-003:48`, any future revision that gives the client a feature request of its own owes this backlog a story and this guarantee a rewrite — both in the same change.
...
5. **No orphan stories.** The seeded backlog contains no story that no transcript, no manual-Appendix exception, or no named legacy-game feature in NAMING_CANON's legacy-game row motivates.
