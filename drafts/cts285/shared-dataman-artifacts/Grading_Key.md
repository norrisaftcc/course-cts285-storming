...
4. **Stretch traceability is not independently confirmed in this checkout.** The backlog cites `CTS285_COURSE_OUTLINE.md` lines 168–185 (S1.1→save/load, S1.2→turn-based battle, S1.3→random encounters & progression, S1.4→creature collection), but that source file is absent from the repository. Keep these stories below the line; restore or rule the source location before grading their traceability.
