...
| Item | Owner |
|---|---|
| QM-reference stripping (assessment-rubrics, COURSEMAP §, README cert line) | QM-retirement task — **EXECUTED 2026-08-07.** Surveyed all three targets: `assessment-rubrics.md` and the README cert line were already clean (stripped by earlier work, unrecorded here), leaving `COURSEMAP.md` as the only file still carrying live QM doctrine. Removed: the §Quality Assurance "22/22 QM standards" claim and its four scored blocks, the QM entry in the Certification Statement, and the QM/CourseForge audits in Planned Updates. Replaced by the four checks `QM_Retirement_Note.md` names as the actual gate, plus an explicit statement that RSI is federal compliance and unaffected. Deferred row kept above rather than deleted, per this ledger's convention. |
...
| COURSEMAP **Module Breakdown** per-assessment/per-component point figures (Week-2 25, per-module "KC … bonus" lines, the broken Sprint-1 component split summing 65) | task **2.5** (COURSEMAP rebuild) — a scope note in COURSEMAP marks the summary table authoritative until then. **Partially executed 2026-08-06 (B-005):** the Week-2 line alone was corrected to `Agile Simulation & Dataman Backlog Initialization (35 pts)` because 1.3 renamed and re-scoped that assessment and could not leave the map promising a deleted deliverable at a wrong value. The per-module KC-bonus lines and the Sprint-1 split summing 65 are untouched and remain 2.5's. |
| ~~Week_02 body Part 2 "Project Selection & Proposal" deletion + paper-sim/seeded-backlog rewrite; the residual 10-pt "Backlog Initialization" rubric line-item criteria~~ **DONE 2026-08-06 (B-005 / task 1.3)** | task **1.3** — ✅ Part 2 deleted; the simulation and board now run on `Dataman_Seeded_Backlog.md`; the 10-pt row is now "Dataman backlog initialization" with line-item criteria (eleven Fibonacci estimates with stated reasons, a ≤13-point justified slice, one filed open question). Rubric split held at the decided 10 / 15 / 10 = 35. Board columns corrected to the canonical four. |
...
| Dataman/Datamon project naming on the generic rubric labels above ("Backlog Initialization", "Grouped Presentation", "Solo arc", module descriptions) — canonical names include "Dataman"; per criterion 6 the sweep left the noun generic | wk-content tasks (1.3 / 1.6 / 2.5) — **1.3's share done 2026-08-06**: the Week_02 rubric label now reads "Dataman backlog initialization". "Grouped Presentation" (1.6) and the module descriptions (2.5) are still generic. |
