...
| R-006 | The lightweight deploy in weeks 11–12 fails with no time to troubleshoot. | Schedule | 3 | 5 | **15 (HIGH)** | Do a throwaway deploy of a "hello Dataman" build by week 8 — two-plus sprints early — so the deploy path is proven before it's on the critical path. Ask Teacherbot to review the deploy plan in week 11. Fallback: demo from localhost if the deploy fails. | Robin | Mitigated | Sprint 1 |
...
- **Escalation**: decide when a risk goes to Teacherbot (as client/instructor) — e.g. score 20+
